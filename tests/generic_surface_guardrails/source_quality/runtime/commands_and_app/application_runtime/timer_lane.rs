...
fn application_runtime_timer_lane_keeps_worker_policy_and_tests_focused() {
    ...
    let queue = fs::read_to_string(manifest_dir.join("src/application/runtime/timer/queue.rs"))
        .expect("application runtime timer queue should be readable");
    let worker = fs::read_to_string(manifest_dir.join("src/application/runtime/timer/worker.rs"))
        .expect("application runtime timer worker should be readable");
    let tests =
    ...
    assert!(
        lane.contains("pub(in crate::application::runtime) struct TimerLane")
    ...
    assert!(
        !lane.contains("Message")
            && !queue.contains("Message")
            && !worker.contains("Message")
            && !queue.contains("mapper")
            && !worker.contains("mapper"),
        "timer lane, queue, and worker must transport only opaque timer identities"
    );
    assert!(
