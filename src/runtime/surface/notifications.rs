...
fn collect<Message>(
    ...
    if let Some(demand) = node.notice_demand() {
        ...
        collect_widgets(node, &mut widgets);
        out.push(ProjectedNoticeDescriptor {
            identity: NoticeOwnerIdentity {
                capability: std::any::TypeId::of::<Message>(),
        ...
        match self {
            Self::Scene(scene) => scene.layers.iter().any(|layer| {
                layer.kind == super::LayerKind::Modal
                    || layer.input.as_ref().is_some_and(SurfaceNode::has_notice_modal)
                    || layer.node.has_notice_modal()
            }) || scene.base.has_notice_modal(),
            Self::Container(container) => container
                .children
                .iter()
                .any(|child| child.child.has_notice_modal()),
...
fn collect_widgets<Message>(node: &SurfaceNode<Message>, out: &mut Vec<WidgetId>) {
    match node {
        SurfaceNode::Widget(widget) => out.push(widget.id()),
        SurfaceNode::Container(container) => {
            for child in &container.children { collect_widgets(&child.child, out); }
        }
        SurfaceNode::FloatingLayer(layer) => {
            for child in &layer.container.children { collect_widgets(&child.child, out); }
        }
        SurfaceNode::Scene(scene) => {
            collect_widgets(&scene.base, out);
            for layer in scene.ordered_layers() {
                if let Some(input) = &layer.input { collect_widgets(input, out); }
                collect_widgets(&layer.node, out);
            }
        }
