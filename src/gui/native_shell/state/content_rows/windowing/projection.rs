...
pub(in crate::gui::native_shell::state) fn content_rows_cache_key(
    ...
    ContentRowsCacheKey {
        ...
        content_row_height: f32_to_bits(sizing.browser_row_height),
        content_row_gap: f32_to_bits(sizing.browser_row_gap),
        content_rows_max_per_column: usize_to_u32(sizing.browser_rows_max_per_column),
        row_capacity,
        content_row_count: rows.len() as u32,
        focused_visible_row,
...
pub(in crate::gui::native_shell::state) fn rendered_content_rows_cached_with_window_start_and_previous(
    ...
    for (row, rect) in window.iter().zip(row_rects) {
        let row_text_layout = compute_content_row_text_layout(rect, sizing);
        ...
        let similarity_button_reserved_width = row_similarity_button_reserved_width(
