...
#[test]
fn gpu_surface_fast_path_respects_horizontal_capability_and_zero_delta() {
    let mut bridge = GpuWheelBridge::default();
    bridge.capabilities.coalesce_horizontal_wheel = true;
    ...
    assert!(!runner.can_coalesce_gpu_surface_wheel(point, Vector2::new(0.0, 0.0)));
}
