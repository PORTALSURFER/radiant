...
impl RasterBuffer {
    ...
    pub(super) fn blend_pixel(&mut self, paint: PixelPaint) {
        let Some(index) = self.pixel_index(paint.x, paint.y) else {
            return;
        };
        let alpha = (paint.color[3] as f32 / 255.0) * paint.alpha.clamp(0.0, 1.0);
        for (channel, source) in paint.color.iter().take(3).enumerate() {
