//! Waveform source loading and fallback construction.

#[cfg(test)]
use super::WaveformSummary;
use super::{
    BAND_COUNT, WaveformFile,
    bands::{downmix_to_mono, interleaved_band_samples, split_frequency_bands},
};
...
pub(crate) fn resolve_sample_path() -> Option<PathBuf> {
    std::env::args_os()
        .nth(1)
        .map(PathBuf::from)
...
pub(crate) fn load_waveform_source(path: Option<PathBuf>) -> Result<WaveformFile, String> {
    match path {
...
pub(crate) fn waveform_file_from_mono_samples(
    path: PathBuf,
    sample_rate: u32,
    channels: usize,
