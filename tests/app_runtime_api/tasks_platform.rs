...
#[test]
fn owner_keyed_latest_ordered_stream_api_preserves_key_ticket_fifo_receipt_and_ui_mappers() {
    let owner = radiant::application::DeclarativeEffectOwner::new();
    let mut keyed = radiant::prelude::KeyedLatestTasks::new();
    let key = String::from("row-1");
    ...
    let request = context
        .business()
        .background("owner-keyed-latest-stream-receipt")
        .latest_for(&mut keyed, key.clone());
    let ticket = request.ticket();
    let receipt = request.stream_for_owner_with_receipt(
        ...
        move |completion| {
            ...
            event_state
                .borrow_mut()
                .push((completion.key, completion.ticket, completion.output));
            DemoMessage::Increment
        },
        move |completion| {
    ...
    assert_worker_command(
        &context.into_command(),
        "owner-keyed-latest-stream-receipt",
        radiant::prelude::TaskPriority::Background,
    );
    assert!(mapped.borrow().is_empty());
}
...
#[test]
fn cancellable_owner_ordered_stream_api_accepts_ui_local_mappers_and_token_clone() {
    let owner = radiant::application::DeclarativeEffectOwner::new();
    let mapped = std::rc::Rc::new(std::cell::RefCell::new(Vec::new()));
    let event_state = std::rc::Rc::clone(&mapped);
    let final_state = std::rc::Rc::clone(&mapped);
    ...
    let request = context
        .business()
        .background("cancellable-owner-stream-receipt")
        .cancellable();
    ...
    assert_worker_command(
        &context.into_command(),
        "cancellable-owner-stream-receipt",
        radiant::prelude::TaskPriority::Background,
    );
    assert!(mapped.borrow().is_empty());
}
...
#[test]
fn cancellable_owner_coalesced_stream_api_accepts_ui_local_mappers_and_token_clone() {
    let owner = radiant::application::DeclarativeEffectOwner::new();
    let mapped = std::rc::Rc::new(std::cell::RefCell::new(Vec::new()));
    let event_state = std::rc::Rc::clone(&mapped);
    let final_state = std::rc::Rc::clone(&mapped);
    ...
    let request = context
        .business()
        .background("cancellable-owner-coalesced-stream-receipt")
        .cancellable();
    let token = request.token();
    let receipt = request.stream_latest_for_owner_with_receipt(
        owner,
    ...
    assert_worker_command(
        &context.into_command(),
        "cancellable-owner-coalesced-stream-receipt",
        radiant::prelude::TaskPriority::Background,
    );
    assert!(mapped.borrow().is_empty());
}
