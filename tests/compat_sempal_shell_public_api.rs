//! Public API coverage for the transitional Sempal compatibility namespace.
...
fn default_app_title_is_a_runtime_compat_alias() {
    ...
    assert!(
        ...
        "the compatibility app-contract module must not own a duplicate default title constant"
    );
}
...
fn app_contract_no_longer_exports_waveform_tempo_parsing_helper() {
    ...
    assert!(
        ...
        "waveform tempo parsing is host-owned DTO helper behavior, not part of the Radiant compat export"
    );
}
