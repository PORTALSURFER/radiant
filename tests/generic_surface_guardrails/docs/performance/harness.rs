...
fn performance_harness_runner_exposes_metrics_baselines_and_listing() {
    ...
    assert!(
        args.contains("--jsonl")
            ...
            && metrics.contains("\\\"group\\\":\\\"{}\\\"")
            && metrics.contains("\\\"total_us\\\"")
            && metrics.contains("\\\"avg_us\\\"")
            && metrics.contains("\\\"p50_us\\\"")
            && metrics.contains("\\\"p95_us\\\"")
            && metrics.contains("\\\"p99_us\\\"")
            && baseline.contains("p50_us")
            && baseline.contains("p95_us")
            && baseline.contains("p99_us")
            && runner.contains("SAMPLE_BATCH_SIZE")
            && runner.contains("samples_us")
            ...
            && metrics.contains("text_storage_allocation_count")
    ...
    assert!(
        catalog.contains("ScenarioSpec::new")
            ...
            && catalog.contains("\"pointer_motion\"")
            && catalog.contains("\"virtual_lists\"")
            && catalog.contains("\"scene_cache\"")
            && catalog.contains("\"text_layout\"")
            && catalog.contains("\"retained_gpu_surfaces\"")
            && catalog.contains("\"frame_cadence\"")
            && catalog.contains("POINTER_COUNTERS")
            && catalog.contains("FRAME_CADENCE_COUNTERS")
            && catalog.contains("ARRANGEMENT_SHELL_REFRESH_COUNTERS")
            && catalog.contains("ARRANGEMENT_SHELL_HOVER_COUNTERS")
            && catalog.contains("\"standalone_gui\"")
            && runner.contains("struct ScenarioSpec")
            && runner.contains("category_filters_from_args")
            && runner.contains("group_filters_from_args")
            && args.contains("category_filters_from_args")
            && args.contains("group_filters_from_args")
            && runner.contains("category_filters")
            && runner.contains("group_filters")
            && runner.contains("category={}")
            && runner.contains("group={}")
            && runner.contains("iterations={}"),
        "perf_harness scenario listing and filters should expose target-area categories, blessed groups, counters, and iteration counts"
    );
}
