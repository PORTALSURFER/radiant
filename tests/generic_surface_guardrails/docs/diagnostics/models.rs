...
fn runtime_diagnostics_models_stay_in_focused_modules() {
    ...
    assert!(
        root.contains("mod frame;")
            ...
            && root.contains("pub use frame::NativeCpuFrameCompletionOutcome")
            && root.contains("pub use frame::NativeCpuFrameObservationDiagnostics")
    ...
    assert!(
        frame.contains("pub struct NativeWindowDiagnosticIdentity")
            ...
            && frame.contains("pub enum NativeCpuFrameCompletionOutcome")
            && frame.contains("pub struct NativeCpuFrameObservationDiagnostics")
