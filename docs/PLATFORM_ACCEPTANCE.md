...
| Deterministic host | `DeterministicHost` drives one production runtime with fixed viewport/environment/time, bounded work, and structured observations; its qualified v1 trace boundary validates canonical input, completion, clock, snapshot, and budget data before factory execution and reports first publication divergence. | C + A | macOS, Windows 11 25H2, Ubuntu Wayland | Required | None for the host contract. Native-only consumers remain outside A and must use their own lane. |
...
| Render-canvas contract and fallback guardrail | The target-only CanvasProgram/CanvasGraph vocabulary, 0.1.x compatibility labels, pre-handoff structural validation, typed diagnostics, and mandatory primitive fallback are checked as source/documentation contracts. | C | macOS, Windows 11 25H2, Ubuntu Wayland | Required boundary/documentation gate | None; C evidence cannot establish adapter compilation, GPU rendering, device recovery, or native acceptance. |
...
| OPT-1407 typed CanvasProgram/CanvasGraph fixture and documentation guardrail | `C/NOT_RUN` for this policy snapshot | A private compile fixture exercises invalid-graph, unsupported-version, missing-capability, and explicit primitive-fallback decisions using the current public RenderCanvas/GpuSurface paint APIs. | This is contract/build evidence only; target-only symbols are not exported, and no adapter handoff, graph compilation, GPU rendering, or recovery acceptance is claimed. |
...
| OPT-1384 | Runtime maintainers | Separate trace consumer; the qualified v1 trace boundary now exists, while deterministic evidence remains distinct from native window/GPU/IME/presentation acceptance. |
...
| OPT-1407 | GPU/API maintainers | Own the target `CanvasProgram`/`CanvasGraph` compatibility decision and its C documentation/guardrail evidence; preserve the 0.1.x RenderCanvas/GpuSurface surface and do not infer runtime, adapter, renderer, or native acceptance from this contract-only work. |
