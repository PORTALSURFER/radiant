...
#[test]
fn presentation_transient_overlay_duplicate_key_replaces_in_place() {
    use radiant::prelude as ui;

    let first = ui::TransientOverlay::new(505_u64)
        .paint_only()
        .when(|_: &mut DemoState| true)
        .fps(24)
        .paint(|_state: &mut DemoState, _context, primitives| {
            primitives.push(PaintPrimitive::FillRect(PaintFillRect {
                widget_id: 10,
                rect: ui::Rect::from_size(4.0, 4.0),
                color: ui::Rgba8::new(255, 0, 0, 255),
    ...
    let distinct = ui::TransientOverlay::new(606_u64)
        .paint_only()
        .when(|_: &mut DemoState| true)
        .fps(30)
        .paint(|_state: &mut DemoState, _context, primitives| {
    ...
    let replacement = ui::TransientOverlay::new(505_u64)
        .paint_only()
        .when(|_: &mut DemoState| true)
        .fps(60)
        .paint(|_state: &mut DemoState, _context, primitives| {
            primitives.push(PaintPrimitive::FillRect(PaintFillRect {
                widget_id: 10,
                rect: ui::Rect::from_size(12.0, 12.0),
                color: ui::Rgba8::new(0, 255, 0, 255),
            }));
        });
    ...
    let bridge = ui::app(DemoState::default())
        ...
        .presentation(
            ui::Presentation::new()
                .transient_overlay(first)
                .transient_overlay(distinct)
                .transient_overlay(replacement),
        )
        .update(|_, _: DemoMessage| {})
        .into_bridge();
    let mut runtime = SurfaceRuntime::new(bridge, Vector2::new(180.0, 48.0));
...
#[test]
fn scene_transient_overlay_samples_stateful_activity_once_and_falls_back_once() {
    use radiant::prelude as ui;
    use std::sync::{
        Arc,
        atomic::{AtomicUsize, Ordering},
    ...
    let painter_calls = Arc::new(AtomicUsize::new(0));
    let bridge = ui::app(DemoState::default())
        .view({
            let predicate_calls = Arc::clone(&predicate_calls);
            let painter_calls = Arc::clone(&painter_calls);
            move |state| {
                let predicate_calls = Arc::clone(&predicate_calls);
                let painter_calls = Arc::clone(&painter_calls);
                ui::scene(ui::text(format!("Frame {}", state.count)).id(10))
    ...
    assert_eq!(predicate_calls.load(Ordering::Relaxed), 1);
    assert_eq!(painter_calls.load(Ordering::Relaxed), 1);
    ...
    assert!(runtime.host_animation_activity().needs_animation());
    assert_eq!(predicate_calls.load(Ordering::Relaxed), 3);
    runtime.host_paint_transient_overlay(
        ui::TransientOverlayContext::new(&plan, Vector2::new(180.0, 48.0), Duration::ZERO),
        &mut overlay,
    );
    assert_eq!(predicate_calls.load(Ordering::Relaxed), 3);
    assert_eq!(painter_calls.load(Ordering::Relaxed), 2);
}
