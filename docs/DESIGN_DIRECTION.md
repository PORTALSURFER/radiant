...
This is a bounded public timer, one-shot business-worker, and ordinary ordered
owner-scoped stream consumer, not the complete target effect model. It adds the
qualified DeclarativeEffectOwner handle, explicit ViewNode/Layer markers,
UiUpdateContext owner-timer methods, the owner-scoped one-shot
BusinessRequest and BusinessLatestRequest methods, and
BusinessRequest::stream_for_owner_with_receipt; runtime EffectOrigin, ledger,
and effect registration remain crate-private. It does not define general effect
ownership, demand/refresh/provider budgets, scheduler
budgets/fairness/queue capacity/wake ordering, latest/coalesced owner
streaming, keyed-latest/resource ownership, platform/renderer/product wiring,
or other effect payload compatibility.
