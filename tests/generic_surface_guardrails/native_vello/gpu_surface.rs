...
fn gpu_surface_render_stats_stay_in_focused_diagnostics_module() {
    ...
    assert!(
        custom_shader.contains("fn render_custom_shader")
            ...
            && custom_shader.contains("custom_shader_surfaces_failed += 1")
            && custom_shader.contains("custom_shader_shader_module_failures += 1")
            && custom_shader.contains("custom_shader_pipeline_failures += 1")
            && custom_shader.contains("custom_shader_binding_failures += 1")
            && custom_shader.contains("device.push_error_scope(wgpu::ErrorFilter::Validation)")
            && custom_shader.contains("device.pop_error_scope()")
