...
fn api_docs_describe_the_structural_boundary_strategy() {
    ...
    for required in [
        ...
        "The prelude boundary is intentionally conservative",
        "Advanced host-control APIs, renderer or windowing implementation details",
        "that explicit import is the signal that the example has moved beyond the common app import set",
        ...
        "`ButtonMessage::Activate { provenance }` and `ActivateWithModifiers { provenance }`",
        "`ButtonMessage::activation_provenance()`",
        "`ButtonWidget` keeps plain `Activate` for pointer and keyboard activation",
        "`IconButtonWidget` keeps `ActivateWithModifiers` for pointer activation",
        "The concise `ButtonBuilder::message`, free `button_message`, `SurfaceNode::button`,",
        "Typed `ButtonBuilder::mapped`/`mapped_with`/`filter_mapped`, free",
        ...
        "`ToggleMessage::ValueChanged { checked, provenance }`",
        "`ToggleBuilder::message_with`/`mapped_with`",
        "`SurfaceNode::toggle_mapped`/`toggle_mapped_with_checked`",
        "`WidgetMessageMapper::toggle`/`toggle_mapped` forward the complete typed",
        "`Accessibility` and `Programmatic` remain explicit direct provenance values",
