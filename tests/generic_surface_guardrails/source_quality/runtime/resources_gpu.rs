...
fn gpu_surface_input_builder_uses_named_parts_for_message_mapping() {
    ...
    let application_builder =
        fs::read_to_string(manifest_dir.join("src/application/builders/leaf.rs"))
            .expect("application leaf builders should be readable");
    let application =
        fs::read_to_string(manifest_dir.join("src/application.rs")).expect("application facade");
    let prelude = public_prelude_source(&manifest_dir);
    let app_test = fs::read_to_string(manifest_dir.join("tests/app_runtime_api/gpu_surface.rs"))
        .expect("app GPU surface API tests should be readable");
