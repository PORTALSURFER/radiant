...
//!
//! M4 keeps this state machine independent from the runtime bridge. A request
//! owns one lowering transaction; bridge wiring consumes its stage decision in
//! the next phase.
...
struct PendingProjection {
    ...
    receipt: Arc<ApplicationProjectionReceipt>,
}
...
enum Phase {
    Fresh,
    StartupUnbound {
        committed: Arc<ApplicationProjectionReceipt>,
...
struct ValidatedRequestTicket {
    ...
    baseline: Arc<ApplicationProjectionReceipt>,
...
pub(crate) struct Candidate<Payload = Arc<ApplicationProjectionReceipt>> {
...
impl ApplicationProjectionProducer {
    ...
    pub(crate) fn commit_startup(&mut self, receipt: Arc<ApplicationProjectionReceipt>) -> bool {
    ...
    fn prepare_ticket(&mut self, request: ProducerRequest) -> Option<ValidatedRequestTicket> {
        ...
        match phase {
            Phase::StartupUnbound {
                ...
                let baseline = Arc::clone(&committed);
        ...
        if stage && ticket.stageable {
            ...
            } = self.guard.phase
            {
                *pending = Some(PendingProjection {
                    ...
                    receipt: Arc::clone(&self.candidate.receipt),
                });
...
mod tests {
    ...
    fn receipt() -> Arc<ApplicationProjectionReceipt> {
        Arc::new(ApplicationProjectionReceipt {
            nodes: Box::new([]),
            supported: true,
            emitted_records: 0,
            comparison_count: 0,
    ...
    fn distinct_receipts_are_retained_and_supplied_as_the_next_baseline() {
        ...
        let mut producer = make_producer(NonZeroU64::new(7));
        let s0 = receipt();
        assert!(producer.commit_startup(Arc::clone(&s0)));
        ...
        assert!(Arc::ptr_eq(&candidate.receipt, &s1));
        ...
        assert!(Arc::ptr_eq(&candidate.receipt, &s2));
        ...
        let (candidate, _) = producer
            ...
            .stage();
        assert!(saw_s2);
        assert!(Arc::ptr_eq(&candidate.receipt, &s3));
    }
