use super::ExampleContract;

pub(super) const CONTRACTS: &[ExampleContract] = &[
    (
        "styling",
        &[
            ".primary()",
            ".danger()",
            ".subtle()",
            ".hoverable()",
            "toggle(",
        ],
    ),
    (
        "theme_playground",
        &[
            "ThemeTokens::dark_for_viewport_width(",
            "effective_ui_scale(",
            "resolve_widget_visual_tokens(",
            "radiant_theme_playground",
        ],
    ),
    (
        "dpi_scaling",
        &[
            "use radiant::theme::DpiScale;",
            "DpiScale::new(",
            "context.set_dpi_scale(",
            "context.set_window_logical_size(",
            "physical_to_logical(",
            "DpiScaleChoice::Two",
            "dpi_scaling_example_projects_metrics_for_selected_scale",
        ],
    ),
    ("scroll", &["scroll_column(", ".fill_height()"]),
    (
        "sizing",
        &[".size(", ".min_size(", ".preferred_size(", ".fill_width()"],
    ),
    (
        "message_routing",
        &[".handle_message", "context.emit", "context.request_repaint"],
    ),
    ("keys", &[".key(", "list_row(", ".reverse()"]),
    (
        "focus_controls",
        &[
            ".shortcuts(",
            "ShortcutResolution::action",
            ".handle_message(",
            "context.focus(",
            "text_input(",
            ".message(",
        ],
    ),
    (
        "plugin_panel",
        &[
            "grid_with_gaps(",
            "toggle(",
            "parameter_tile(",
            "WidgetProminence::Subtle",
        ],
    ),
    (
        "eq_editor",
        &[
            "Graphical EQ",
            "custom_widget_mapped(",
            "EqEditorMessage::MoveBand",
            "examples/eq_editor/widget/geometry.rs",
            "examples/eq_editor/widget/paint.rs",
            "examples/eq_editor/widget/paint/grid.rs",
            "examples/eq_editor/widget/paint/primitives.rs",
            "examples/eq_editor/widget/response.rs",
            "PaintPrimitive::StrokePolyline",
            "prefers_pointer_move_paint_only",
            "append_runtime_overlay_paint",
            "without_dsp",
        ],
    ),
    (
        "custom_widget",
        &[
            "use radiant::prelude::*;",
            "fn append_paint(",
            "impl Widget for StatusChip",
            "WidgetOutput::custom(",
            "dispatch_input(",
        ],
    ),
    (
        "render_canvas",
        &[
            "render_canvas_input(",
            "RenderCanvasContent::RgbaAtlas",
            "OnceLock<Arc<ImageRgba>>",
            "render_canvas_example_lowers_to_retained_gpu_primitive",
            "render_canvas_example_routes_input_to_state",
        ],
    ),
    (
        "custom_shader_surface",
        &[
            "render_canvas(",
            "RenderCanvasShaderSurfaceDescriptor::new",
            "RenderCanvasContent::CustomShader",
            "custom_shader_surface_example_lowers_to_render_canvas_primitive",
            "custom_shader_surface_example_descriptor_is_valid",
        ],
    ),
    (
        "render_canvas_stack_overlay",
        &[
            "render_canvas(",
            "custom_widget_mapped(",
            ".animated_transient_overlay_at(",
            "SurfacePaintPlan",
            "SelectionOverlay",
            "examples/render_canvas_stack_overlay/tests.rs",
        ],
    ),
    (
        "macos_frame_profile_acceptance",
        &[
            "use radiant::prelude::*;",
            "ProfilingOptions",
            ".profiling(",
            ".animation(|_| true)",
            ".on_frame(|| AcceptanceMessage::FrameTick)",
            ".on_frame_profile(",
            ".auxiliary_windows(",
            "AuxiliaryWindow::utility(",
            ".on_close(AcceptanceMessage::CloseAuxiliary)",
            "Arc::new(",
            "--main=",
            "--aux=",
            "FrameProfileGpuTimingStatus::Unavailable",
            "sequence_violations",
            "Record click",
            "Record auxiliary click",
            "FrameTick",
            "CloseAuxiliary",
            "auxiliary_visible",
            "auxiliary_window_is_ready",
            "combined_frame_auxiliary_projection_waits_for_primary_identity",
            "resize the primary window",
            "#[cfg(target_os = \"macos\")]",
            "#[cfg(test)]",
        ],
    ),
    (
        "macos_devtools_acceptance",
        &[
            "use radiant::prelude::*;",
            "DevtoolsOverlayOptions",
            ".devtools_overlay(DevtoolsOverlayOptions::enabled())",
            ".size(1120, 680)",
            ".min_size(860, 460)",
            "button(\"Primary action\")",
            "button(\"Secondary action\")",
            "text_input(",
            "toggle(\"Enabled\"",
            "InputChanged(String)",
            "ToggleChanged(bool)",
            "MAX_INPUT_CHARS: usize = 64",
            ".chars().take(MAX_INPUT_CHARS)",
            "observational runtime paint only",
            "macOS live devtools acceptance",
            "selected-node metadata",
            "resizing updates the selected bounds",
            "#[cfg(target_os = \"macos\")]",
            "#[cfg(test)]",
            "macos_devtools_acceptance is macOS-only",
        ],
    ),
    (
        "macos_external_drag_acceptance",
        &[
            "use radiant::prelude::*;",
            "DragPreview",
            "DragRequest::new(",
            "ExternalDragRequest::files(",
            "context.begin_drag_with_external(",
            "ExternalDragCompleted(std::result::Result<ExternalDragOutcome, String>)",
            "MAX_CALLBACK_COUNT: u32 = 8",
            "callback_count",
            "terminal_effect",
            "accepted",
            "callback_terminal",
            "bounded_status",
            "DisposableSource",
            "#[cfg(target_os = \"macos\")]",
            "#[cfg(not(target_os = \"macos\"))]",
            "#[cfg(test)]",
            "macos_external_drag_acceptance is macOS-only",
            "acceptance_surface_arms_external_drag_through_update_context",
            "acceptance_state_reports_terminal_copy_and_bounds_callback_count",
            "acceptance_state_bounds_terminal_error_status",
        ],
    ),
    (
        "macos_numeric_accessibility_acceptance",
        &[
            "use radiant::prelude::*;",
            "radiant::app(AcceptanceState::default())",
            ".view(project_surface)",
            ".update(update)",
            ".into_bridge()",
            "SurfaceRuntime::new(",
            ".automation_target_snapshot()",
            "AutomationRole::TextInput",
            "authority.materialized",
            "numeric_input(",
            ".on_interaction_with_accessibility(",
            "NumericAccessibilityOutcome",
            "AcceptanceMessage::Accessibility",
            "AXIncrement",
            "AXDecrement",
            "production_stack_publishes_one_materialized_numeric_target",
            "application_update_reduces_a_native_edit_outcome",
            "#[cfg(target_os = \"macos\")]",
            "#[cfg(not(target_os = \"macos\"))]",
            "#[cfg(test)]",
            "macos_numeric_accessibility_acceptance is macOS-only",
            "do not synthesize an accessibility element or claim live AppKit evidence",
        ],
    ),
    (
        "macos_text_input_ime_acceptance",
        &[
            "use radiant::prelude::*;",
            "radiant::app(AcceptanceState::default())",
            ".view(project_surface)",
            ".update(update)",
            ".on_startup(|_, context| context.focus(TEXT_INPUT_ID))",
            "text_input(state.value.clone())",
            ".message_event(AcceptanceMessage::TextInput)",
            "dispatch_focused_composition_sample",
            "CompositionSample::update(",
            "CompositionSample::commit(",
            "CompositionSample::cancel()",
            "changed_messages",
            "set_ime_cursor_area",
            "candidate panel",
            "preedit",
            "focus loss",
            "preedit_stays_widget_local_without_application_state_change",
            "commit_replaces_once_and_emits_one_changed_message",
            "disabled_cancel_restores_committed_state_without_a_change",
            "focus_loss_restores_committed_state_without_a_change",
            "focused_text_input_publishes_the_caret_area_source_for_native_ime",
            "#[cfg(target_os = \"macos\")]",
            "#[cfg(not(target_os = \"macos\"))]",
            "#[cfg(test)]",
            "macos_text_input_ime_acceptance is macOS-only",
            "does not claim live AppKit evidence",
        ],
    ),
];
