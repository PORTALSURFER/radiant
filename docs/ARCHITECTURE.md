...
owner-scoped stream consumers, and ordered latest-task owner streams reuse this
...
and ordered latest-task owner streams are shipped; coalesced latest-task owner
streams, keyed-latest/resource ownership, platform ownership, and broader product-facing
...
1. Declarative lowering and traversal preserve crate-private source metadata
   ...
   methods, and qualified one-shot/ordinary ordered/coalesced/ordered-latest
...
4. The existing timer and worker registries carry the explicitly selected owner
   ...
   ordered latest-task owner streams. Coalesced latest-task owner streams,
   keyed-latest/resource ownership, platform ownership, and product wiring
   remain deferred. The existing registries
