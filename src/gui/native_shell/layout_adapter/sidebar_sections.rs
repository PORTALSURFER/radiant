...
fn resolve_pane_heights(
    ...
    let source_demand_height =
        stack_height(source_rows, sizing.source_row_height, sizing.source_row_gap);
    let source_min_rows = if source_rows == 0 {
        0
    } else {
        source_rows.min(sizing.source_rows_min_when_split).max(1)
    };
    ...
    let source_gap = if source_rows > 0 {
