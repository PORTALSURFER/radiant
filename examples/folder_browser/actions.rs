//! User actions and state transitions for the folder browser example.
...
impl BrowserState {
    pub(super) fn select_folder(&mut self, id: impl Into<String>) {
    ...
    pub(super) fn activate_folder(&mut self, id: impl Into<String>) {
        let id = id.into();
    ...
    #[cfg(test)]
    pub(super) fn toggle_folder(&mut self, id: impl Into<String>) {
        let id = id.into();
    ...
    pub(super) fn select_file_id(&mut self, id: String) {
    ...
    pub(super) fn open_context_menu_at(&mut self, id: String, position: radiant::layout::Point) {
    ...
    pub(super) fn open_file_context_menu_at(
        &mut self,
        id: String,
    ...
    pub(super) fn close_file_context_menu(&mut self) {
        self.context_file = None;
        self.context_position = None;
    }

    pub(super) fn open_column_context_menu_at(
        &mut self,
        id: String,
        position: radiant::layout::Point,
    ) {
    ...
    pub(super) fn close_column_context_menu(&mut self) {
        self.context_column = None;
        self.context_position = None;
    }

    pub(super) fn create_folder_from_context(&mut self) {
    ...
    pub(super) fn create_file_in_selected_folder(&mut self) {
        let Some(root_id) = self.folders.first().map(|folder| folder.id.clone()) else {
            return;
        };
    ...
    pub(super) fn begin_rename_from_context(&mut self) {
    ...
    pub(super) fn cancel_renames(&mut self) {
        self.cancel_folder_rename();
        self.cancel_file_rename();
    }

    pub(super) fn cancel_folder_rename(&mut self) {
        self.rename_folder = None;
        self.rename_draft.clear();
    }

    pub(super) fn cancel_file_rename(&mut self) {
    ...
    pub(super) fn begin_file_rename_from_context(&mut self) {
    ...
    pub(super) fn commit_file_rename(&mut self) {
    ...
    pub(super) fn delete_file_from_context(&mut self) {
        let Some(file_id) = self.context_file.take() else {
            return;
        };
        let Some(root_id) = self.folders.first().map(|folder| folder.id.clone()) else {
    ...
    pub(super) fn sort_by(&mut self, column_id: String) {
    ...
    pub(super) fn toggle_file_column(&mut self, column_id: String) {
    ...
    pub(super) fn reset_file_columns(&mut self) {
    ...
    pub(super) fn resize_file_column(&mut self, column_id: String, message: ui::DragHandleMessage) {
        match message {
    ...
    pub(super) fn resize_tree(&mut self, message: ui::DragHandleMessage) {
        match message {
            ui::DragHandleMessage::Started { position }
            | ui::DragHandleMessage::Moved { position }
            | ui::DragHandleMessage::Ended { position } => {
    ...
    pub(super) fn handle_folder_drag(&mut self, source_id: String, message: ui::DragHandleMessage) {
        match message {
            ui::DragHandleMessage::Started { position } => {
